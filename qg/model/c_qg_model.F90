...
call calc_pv(flds%geom%nx,flds%ny,flds%q,flds%x,flds%x_north,flds%x_south, &
           & conf%f1,conf%f2,conf%deltax,conf%deltay,bet,conf%rs)
call zonal_wind(flds%u,flds%x,flds%x_north,flds%x_south,flds%geom%nx,flds%ny, &
              & conf%deltay)
call meridional_wind(flds%v,flds%x,flds%geom%nx,flds%ny,conf%deltax)
...
call meridional_wind_ad(flds%v,flds%x,flds%geom%nx,flds%ny, conf%deltax)
call zonal_wind_ad(flds%u,flds%x,flds%geom%nx,flds%ny,conf%deltay)
call calc_pv_ad(flds%q,flds%x,flds%geom%nx,flds%ny, &
...
call calc_pv_tl(flds%q,flds%x,flds%geom%nx,flds%ny, &
              & conf%f1,conf%f2,conf%deltax,conf%deltay)
call zonal_wind_tl(flds%u,flds%x,flds%geom%nx,flds%ny,conf%deltay)
call meridional_wind_tl(flds%v,flds%x,flds%geom%nx,flds%ny, conf%deltax)
...
allocate(qnew(flds%geom%nx,flds%ny,2))
allocate(x_traj(flds%geom%nx,flds%ny,2))
allocate(qn_traj(flds%geom%nx,2))
allocate(qs_traj(flds%geom%nx,2))
allocate(q_traj(flds%geom%nx,flds%ny,2))
allocate(u_traj(flds%geom%nx,flds%ny,2))
allocate(v_traj(flds%geom%nx,flds%ny,2))
call get_traj(traj,flds%geom%nx,flds%ny,x_traj,xn_traj,xs_traj,qn_traj,qs_traj)

!--- generate trajectory values for potential vorticity and wind

call calc_pv(flds%geom%nx,flds%ny,q_traj,x_traj,xn_traj,xs_traj, &
    &        conf%f1,conf%f2,conf%deltax,conf%deltay,bet,conf%rs)
call zonal_wind (u_traj,x_traj,xn_traj,xs_traj,flds%geom%nx,flds%ny, conf%deltay)
call meridional_wind (v_traj,x_traj,flds%geom%nx,flds%ny,conf%deltax)
...
call invert_pv_ad(flds%x,qnew,flds%geom%nx,flds%ny, &
...
call advect_pv_ad(qnew,flds%q,q_traj,qn_traj,qs_traj, &
    &             flds%u,u_traj,flds%v,v_traj,flds%geom%nx,flds%ny, &
...
call invert_pv_tl(flds%x,qnew,flds%geom%nx,flds%ny, &
                & conf%deltax,conf%deltay,conf%f1,conf%f2)
...
call qg_traj_registry%get(c_key_traj,traj)
call set_traj(traj,flds%geom%nx,flds%ny, &
