...
subroutine qg_fields_interp(fld,locs,vars,gom)
...
! Local variables
integer :: jloc,ival(locs%nlocs)
real(kind_real),allocatable :: x(:,:,:),q(:,:,:),u(:,:,:),v(:,:,:)

! Check field
call qg_fields_check(fld)

! Check vars/gom consistency
if (vars%lx.neqv.(gom%ix>0)) call abor1_ftn ('qg_fields_interp: inconsistent vars and gom')
if (vars%lq.neqv.(gom%iq>0)) call abor1_ftn ('qg_fields_interp: inconsistent vars and gom')
if (vars%lu.neqv.(gom%iu>0)) call abor1_ftn ('qg_fields_interp: inconsistent vars and gom')
if (vars%lv.neqv.(gom%iv>0)) call abor1_ftn ('qg_fields_interp: inconsistent vars and gom')
...
if (vars%lx.or.vars%lu.or.vars%lv) then
...
if (vars%lu.or.vars%lv) call convert_x_to_uv(fld%geom,x,fld%x_north,fld%x_south,u,v)
...
do jloc=1,locs%nlocs
  ...
  if (vars%lx) call qg_interp_trilinear(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),x,gom%values(gom%ix,ival(jloc)))
  if (vars%lq) call qg_interp_trilinear(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),q,gom%values(gom%iq,ival(jloc)))
  if (vars%lu) call qg_interp_trilinear(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),u,gom%values(gom%iu,ival(jloc)))
  if (vars%lv) call qg_interp_trilinear(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),v,gom%values(gom%iv,ival(jloc)))
enddo
!$omp end parallel do

end subroutine qg_fields_interp
! ------------------------------------------------------------------------------
!> Interpolation from fields - tangent linear
subroutine qg_fields_interp_tl(fld,locs,vars,gom)
...
if (vars%lu.or.vars%lv) call convert_x_to_uv_tl(fld%geom,x,u,v)
...
end subroutine qg_fields_interp_tl
! ------------------------------------------------------------------------------
!> Interpolation from fields - adjoint
subroutine qg_fields_interp_ad(fld,locs,vars,gom)
...
do jloc=locs%nlocs,1,-1
  ! Get gom%used
  gom%used = gom%used+1
  ival(jloc) = gom%nobs-gom%used+1
end do

do jloc=locs%nlocs,1,-1
  ! Interpolate variables
  if (vars%lx) call qg_interp_trilinear_ad(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),gom%values(gom%ix,ival(jloc)),x)
  if (vars%lq) call qg_interp_trilinear_ad(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),gom%values(gom%iq,ival(jloc)),q)
  if (vars%lu) call qg_interp_trilinear_ad(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),gom%values(gom%iu,ival(jloc)),u)
  if (vars%lv) call qg_interp_trilinear_ad(fld%geom,locs%lon(jloc),locs%lat(jloc),locs%z(jloc),gom%values(gom%iv,ival(jloc)),v)
enddo

! Get variables
if (vars%lu.or.vars%lv) call convert_x_to_uv_ad(fld%geom,u,v,x)
if (vars%lq) then
