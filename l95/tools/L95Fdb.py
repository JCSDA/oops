...
class L95Fdb:
  ...
  def __init__(self, fdbdir):
    ...
    for i in l:
      ...
      if file["term"] is None:
        key = file["expe"]+"."+file["type"]+"."+file["referenceTime"]
      else:
        key = file["expe"]+"."+file["type"]+"."+file["referenceTime"]+"."+file["term"]
      self.map[key] = file
  ...
  def getData(self, file):
    if "data" in file:
       ...
       d = np.fromfile(file=infile, dtype="float64", \
                      count=int(n), sep=" ")
       infile.close()
       file["data"] = d
    return d
