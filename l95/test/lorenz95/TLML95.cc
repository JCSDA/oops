...
// -----------------------------------------------------------------------------
  SECTION("test_tlmL95_constructor") {
    boost::scoped_ptr<lorenz95::TLML95> tlm(new lorenz95::TLML95(*f.resol_, *f.tlconf_));
    EXPECT(tlm.get() != NULL);
  }
// -----------------------------------------------------------------------------
/*
  SECTION("test_tlmL95_set_get_Trajectory") {
    lorenz95::TLML95 tlm(*f.resol_, *f.tlconf_, *f.model_);

    // construct a StateL95 object
    std::string date_string("2014-09-12T09:35:00Z");
    util::DateTime dt(date_string);
    oops::Variables vars();
    boost::scoped_ptr<lorenz95::StateL95> stateL95(new lorenz95::StateL95(*f.resol_, vars, dt));
    ...
    boost::scoped_ptr<lorenz95::ModelBias> modelBias(new lorenz95::ModelBias(*biasCfg, *f.resol_));
...
/*
  SECTION("test_tlmL95_stepTL") {
    ...
    boost::scoped_ptr<lorenz95::FieldL95> fieldL95(new lorenz95::FieldL95(*f.resol_));
    ...
    boost::scoped_ptr<lorenz95::ModelBiasCovariance> modelBiasCovariance(
        new lorenz95::ModelBiasCovariance(*covarCfg, *f.resol_));
...
// -----------------------------------------------------------------------------
  SECTION("test_tlmL95_get_classname") {
    lorenz95::TLML95 tlm(*f.resol_, *f.tlconf_);
    EXPECT(tlm.classname() == "lorenz95::TLML95");
  }
// -----------------------------------------------------------------------------
  SECTION("test_tlmL95_get_timestep") {
    lorenz95::TLML95 tlm(*f.resol_, *f.tlconf_);
    util::Duration dt(f.tlconf_->getString("tstep"));
