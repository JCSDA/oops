...
class ObsErrorFactory {
 ...
 public:
  ...
  ///
  /// The model's type is determined by the `covariance model` determined by
  /// the \p eckit::Configuration (default `covariance model` is `diagonal`)
  static std::unique_ptr<ObsErrorBase_> create(const eckit::Configuration & conf,
                                               const ObsSpace_ &);
 ...
 private:
  virtual std::unique_ptr<ObsErrorBase_> make(const eckit::Configuration &,
                                              const ObsSpace_ &) = 0;
...
class ObsErrorMaker : public ObsErrorFactory<OBS> {
  ...
  std::unique_ptr<ObsErrorBase_> make(const eckit::Configuration & config,
                                      const ObsSpace_ & obs) override {
    return std::make_unique<T>(config, obs);
  }
