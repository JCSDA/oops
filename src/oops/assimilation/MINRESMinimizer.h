...
template<typename MODEL, typename OBS> class MINRESMinimizer : public PrimalMinimizer<MODEL, OBS> {
...
double MINRESMinimizer<MODEL, OBS>::solve(CtrlInc_ & dx, const CtrlInc_ & rhs,
                                  const Hessian_ & hessian, const Pmat_ & P,
                                  const int ninner, const double gnreduc) {
// Solve the linear system
  const double reduc = MINRES(dx, rhs, hessian, P, ninner, gnreduc);
  return reduc;
}
