...
!----------------------------------------------------------------------
subroutine stripack_addnod(nst,k,x,y,z,list,lptr,lend,lnew)
...
! Initialization
kk = k
if (kk<4) call abor1_ftn('stripack_addnod: K<4')
km1 = kk-1
ist = nst
if (ist<1) ist = km1
p(1) = x(kk)
...
if (i1==0) call abor1_ftn('stripack_addnod: colinear points')
if (i3/=0) then
  l = i1
...
!----------------------------------------------------------------------
subroutine stripack_trmesh(n,x,y,z,list,lptr,lend,lnew,near,next,dist)
...
! Initialization
nn = n
if (nn<3) call abor1_ftn('stripack_trmesh: N < 3')
list = 0
lptr = 0
lend = 0
...
do k=4,nn
   call addnod(near(k),k,x,y,z,list,lptr,lend,lnew)
