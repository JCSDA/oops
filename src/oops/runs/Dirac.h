...
template <typename MODEL> class DiracParameters : public ApplicationParameters {
 ...
 public:
  ...
  /// Whether and where to write the increments generated by the randomization.
  OptionalParameter<eckit::LocalConfiguration> outputEnsemble{"output ensemble", this};
};
...
//  Variance randomization
    ...
    if (outputVariance != boost::none) {
      ...
      // Randomization
      const boost::optional<eckit::LocalConfiguration> &outputEnsemble =
              params.outputEnsemble.value();
      for (size_t ie = 0; ie < Bmat->randomizationSize(); ++ie) {
        ...
        if (outputEnsemble != boost::none) {
          const size_t outputSize = outputEnsemble->getInt("size", Bmat->randomizationSize());
          if (ie < outputSize) {
            Log::info() << "Writing random vector #" << ie + 1 << "." << std::endl;
            auto outputMember(outputEnsemble->getSubConfiguration("member template"));
            const std::string & mb_pattern = outputEnsemble->getString("member pattern");
            util::seekAndReplace(outputMember, mb_pattern, std::to_string(ie + 1));
            if (outputEnsemble->has("mpi pattern")) {
              const std::string & mpi_pattern = outputEnsemble->getString("mpi pattern");
              std::string mpi_size = std::to_string(oops::mpi::world().size());
              util::seekAndReplace(outputMember, mpi_pattern, mpi_size);
            }
            dx.write(outputMember);
          }
        }
