...
template <typename MODEL> void testErrorCovarianceZero() {
  ...
  oops::Log::info() << "dx2.norm()=" << dx2[0].norm() << std::endl;
...
template <typename MODEL> void testErrorCovarianceSym() {
  ...
  oops::Log::info() << "<dx,Bdy>-<Bdx,dy>/<dx,Bdy>=" <<  (zz1-zz2)/zz1 << std::endl;
  oops::Log::info() << "<dx,Bdy>-<Bdx,dy>/<Bdx,dy>=" <<  (zz1-zz2)/zz2 << std::endl;
  const double tol = Test_::test().getDouble("tolerance");
